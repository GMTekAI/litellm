...
def _fake_request():
    return type(
        "Req",
        (),
        {"base_url": URL(PROXY_BASE_URL + "/"), "query_params": {}},
    )()


async def _acs(b64, cache):
    return await SAMLAuthHandler.handle_acs(
        _fake_request(), cache, {"SAMLResponse": b64}
    )
...
@pytest.mark.asyncio
async def test_valid_idp_initiated_login_maps_assertion_to_user(saml_env):
...
async def test_sp_initiated_known_request_succeeds_once_then_replay_rejected(saml_env):
    ...
    result = await _acs(_b64(resp), cache)
    assert result.email == "alice@example.com"
...
@pytest.mark.asyncio
async def test_idp_initiated_assertion_replay_is_rejected(saml_env):
    key_pem, cert_pem = saml_env
    cache = DualCache()
...
@pytest.mark.asyncio
async def test_custom_email_attribute_override(saml_env, monkeypatch):
    key_pem, cert_pem = saml_env
...
@pytest.mark.asyncio
async def test_team_ids_extracted_from_groups_attribute(saml_env):
    key_pem, cert_pem = saml_env
    resp = _build_signed_response(
        key_pem,
        cert_pem,
