...
class BedrockGuardrail(CustomGuardrail, BaseAWSLLM):
    ...
    def _normalize_checks(checks: object | None) -> dict[str, Any] | None:
        ...
        if unknown_keys and cleaned:
            verbose_proxy_logger.warning(
                "BedrockGuardrail: unrecognized check key(s) %s will be ignored; "
                "continuing in checks mode with %s. Known keys: %s.",
                sorted(unknown_keys),
                sorted(cleaned.keys()),
            ...
            verbose_proxy_logger.warning(
                "BedrockGuardrail: unrecognized check key(s) %s will be ignored and no valid "
                "checks remain, so the guardrail falls back to ApplyGuardrail mode. Known keys: %s.",
                sorted(unknown_keys),
                sorted(_BEDROCK_CHECKS_KNOWN_KEYS),
            )
