...
class SAMLAuthHandler:
    ...
    def is_saml_configured() -> bool:
        return bool(
            SAMLAuthHandler._env("SAML_IDP_METADATA_URL")
            or SAMLAuthHandler._env("SAML_IDP_METADATA_XML")
        )
    ...
    def _sp_entity_id(request: Request) -> str:
        return SAMLAuthHandler._env(
            "SAML_SP_ENTITY_ID"
        ) or SAMLAuthHandler._metadata_url(request)
    ...
    async def _load_idp_settings(cache: DualCache) -> dict[str, object]:
        ...
        cache_key = (
            f"{_SAML_IDP_SETTINGS_CACHE_PREFIX}:"
        ...
        cache.set_cache(
            key=cache_key, value=idp_settings, ttl=_SAML_IDP_METADATA_TTL_SECONDS
        )
        return idp_settings

    @staticmethod
    def _build_settings(
        request: Request, idp_settings: dict[str, object]
    ) -> dict[str, object]:
        sp_settings: dict[str, object] = {
            ...
            "security": {
                "wantAssertionsSigned": SAMLAuthHandler._bool_env(
                    "SAML_WANT_ASSERTIONS_SIGNED", True
                ),
                "wantMessagesSigned": SAMLAuthHandler._bool_env(
                    "SAML_WANT_MESSAGES_SIGNED", False
                ),
                "authnRequestsSigned": SAMLAuthHandler._bool_env(
        ...
        return OneLogin_Saml2_IdPMetadataParser.merge_settings(
            sp_settings, idp_settings
        )

    @staticmethod
    def _prepare_request_data(
        request: Request, post_data: Optional[dict[str, str]] = None
    ) -> dict[str, object]:
    ...
    async def handle_acs(
        request: Request, cache: DualCache, post_data: dict[str, str]
    ) -> CustomOpenID:
        ...
        await SAMLAuthHandler._enforce_replay_protection(auth, cache)
    ...
    @staticmethod
    async def _enforce_replay_protection(
        auth: "OneLogin_Saml2_Auth", cache: DualCache
    ) -> None:
        ...
        elif not SAMLAuthHandler._bool_env("SAML_ALLOW_UNSOLICITED", True):
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    def _result_from_auth(auth: "OneLogin_Saml2_Auth") -> CustomOpenID:
        ...
        email = SAMLAuthHandler._attribute_value(
            attributes, "SAML_ATTRIBUTE_EMAIL", _EMAIL_ATTRIBUTE_CANDIDATES
        )
        if email is None and name_id is not None and "@" in name_id:
            email = name_id

        user_id = (
            SAMLAuthHandler._attribute_value(attributes, "SAML_ATTRIBUTE_USER_ID", ())
            or name_id
            or email
        )
        if user_id is None:
        ...
        role_value = SAMLAuthHandler._attribute_value(
        ...
        display_name = (
            " ".join(part for part in (first_name, last_name) if part) or email
        )

        verbose_proxy_logger.info(
            f"SAML login: subject={user_id}, email={email}, attributes={list(attributes.keys())}"
        )
